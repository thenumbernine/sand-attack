...
function PlayerKeysEditor:init(app)
	...
	if not defaultKeys then
		...
		defaultKeys = {
			{
				up = {self.sdlEventKeyDown, sdl.SDLK_UP},
				down = {self.sdlEventKeyDown, sdl.SDLK_DOWN},
				left = {self.sdlEventKeyDown, sdl.SDLK_LEFT},
				right = {self.sdlEventKeyDown, sdl.SDLK_RIGHT},
				pause = {self.sdlEventKeyDown, sdl.SDLK_ESCAPE},
			},
			{
				up = {self.sdlEventKeyDown, ('w'):byte()},
				down = {self.sdlEventKeyDown, ('s'):byte()},
				left = {self.sdlEventKeyDown, ('a'):byte()},
				right = {self.sdlEventKeyDown, ('d'):byte()},
				...
				then
					app.waitingForEvent = {
						...
						callback = function(ev)
							...
							if ev[1] == self.sdlEventKeyDown and ev[2] == sdl.SDLK_ESCAPE then
